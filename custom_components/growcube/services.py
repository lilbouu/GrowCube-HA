...
from .const import DOMAIN, CHANNEL_NAME, SERVICE_WATER_PLANT, \
    SERVICE_SET_SCHEDULED_WATERING, SERVICE_SET_SMART_WATERING, SERVICE_DELETE_WATERING, \
    SERVICE_ADD_PLANT, SERVICE_END_PLANT, SERVICE_REQUEST_HISTORY, ARGS_CHANNEL, ARGS_DURATION, \
    ARGS_INTERVAL, ARGS_PLANT_NAME, ARGS_PHOTO_URL, ARGS_WATERING_MODE, ARGS_MIN_MOISTURE, \
    ARGS_MAX_MOISTURE, ARGS_ALL_DAY, ARGS_START_TIME
...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def async_call_add_plant_service(service_call: ServiceCall) -> None:
        await _async_handle_add_plant(hass, service_call.data)

    async def async_call_end_plant_service(service_call: ServiceCall) -> None:
        await _async_handle_end_plant(hass, service_call.data)

    async def async_call_request_history_service(service_call: ServiceCall) -> None:
        await _async_handle_request_history(hass, service_call.data)
                                 ...
                                 schema=vol.Schema(
                                     {
                                         ...
                                         vol.Optional(ARGS_START_TIME): cv.string,
                                     }
                                 ))
    hass.services.async_register(DOMAIN,
                                 ...
                                 schema=vol.Schema(
                                     {
                                         vol.Required(ATTR_DEVICE_ID): cv.string,
                                         vol.Required(ARGS_CHANNEL, default='A'): cv.string,
                                         vol.Required(ARGS_MIN_MOISTURE, default=20): cv.positive_int,
                                         vol.Required(ARGS_MAX_MOISTURE, default=60): cv.positive_int,
    ...
    hass.services.async_register(DOMAIN,
                                 SERVICE_ADD_PLANT,
                                 async_call_add_plant_service,
                                 schema=vol.Schema(
                                     {
                                         vol.Required(ATTR_DEVICE_ID): cv.string,
                                         vol.Required(ARGS_CHANNEL, default='A'): cv.string,
                                         vol.Optional(ARGS_PLANT_NAME): cv.string,
                                         vol.Optional(ARGS_PHOTO_URL): cv.string,
                                         vol.Optional(ARGS_WATERING_MODE, default="Disabled"): vol.In(
                                             ["Disabled", "Repeating", "Smart"]
                                         ),
                                     }
                                 ))
    hass.services.async_register(DOMAIN,
                                 SERVICE_END_PLANT,
                                 async_call_end_plant_service,
                                 schema=vol.Schema(
                                     {
                                         vol.Required(ATTR_DEVICE_ID): cv.string,
                                         vol.Required(ARGS_CHANNEL, default='A'): cv.string,
    ...
    hass.services.async_register(DOMAIN,
                                 ...
                                 async_call_request_history_service,
                                 schema=vol.Schema(
                                     {
                                         vol.Required(ATTR_DEVICE_ID): cv.string,
                                         vol.Required(ARGS_CHANNEL, default='A'): cv.string,
...
async def _async_handle_set_scheduled_watering(hass: HomeAssistant, data: Mapping[str, Any]) -> None:
    ...
    first_watering_time = _parse_optional_time(data.get(ARGS_START_TIME))
    ...
    channel = Channel(CHANNEL_NAME.index(channel_str))
    await coordinator.handle_set_scheduled_watering(channel, duration, interval, first_watering_time)
...
def _parse_optional_time(value: Any) -> time | None:
    ...
    try:
        ...
        raise HomeAssistantError(f"Invalid start_time '{value}' specified, expected HH:MM or HH:MM:SS") from err
...
async def _async_handle_set_smart_watering(hass: HomeAssistant, data: Mapping[str, Any]) -> None:
    ...
    if min_moisture < 1 or min_moisture > 99:
        _LOGGER.error(
    ...
    if max_moisture < 1 or max_moisture > 99:
        _LOGGER.error(
    ...
    if min_moisture >= max_moisture:
        _LOGGER.error(
            "%s: %s - Invalid moisture range, max_moisture %s must be bigger than min_moisture %s",
            device,
            SERVICE_SET_SMART_WATERING,
            max_moisture,
            min_moisture
        ...
        raise HomeAssistantError(
            f"Invalid values specified, max_moisture {max_moisture} must be bigger than "
...
async def _async_handle_add_plant(hass: HomeAssistant, data: Mapping[str, Any]) -> None:
    coordinator, device = _get_coordinator(hass, data)
    ...
    }[data[ARGS_WATERING_MODE]]
    await coordinator.async_add_plant(
        CHANNEL_NAME.index(channel_str),
        data.get(ARGS_PLANT_NAME),
        data.get(ARGS_PHOTO_URL),
        mode,
    )


async def _async_handle_end_plant(hass: HomeAssistant, data: Mapping[str, Any]) -> None:
    coordinator, device = _get_coordinator(hass, data)
    ...
    await coordinator.async_reset_plant(CHANNEL_NAME.index(channel_str))


async def _async_handle_request_history(hass: HomeAssistant, data: Mapping[str, Any]) -> None:
    coordinator, device = _get_coordinator(hass, data)
    ...
    await coordinator.async_request_history(CHANNEL_NAME.index(channel_str))
