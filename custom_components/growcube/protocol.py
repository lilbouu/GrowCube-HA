"""GrowCube ELEA protocol helpers."""
...
class EleaMessage:
    """Parsed GrowCube protocol message."""
...
def build_message(command: int, payload: str | None = None) -> bytes:
    """Build a GrowCube ELEA protocol message."""
...
def parse_messages(buffer: bytearray) -> list[EleaMessage]:
    """Parse complete ELEA messages from a byte buffer and consume them."""
...
def channel_payload(channel: int) -> str:
    """Build a single-channel payload."""
...
def time_sync_payload(value: datetime) -> str:
    """Build command 44 payload."""

    return value.strftime("%Y@%m@%d@%H@%M@%S")


def growcube_local_epoch(value: datetime) -> int:
    """Return the local-component epoch expected by GrowCube command 51."""
